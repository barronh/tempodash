...
class report:
    def __init__(self, xkey, ykey, meta=None):
        ...
        ykey : str
            tempo_no2_sum, tempo_no2_trop, tempo_hcho_total, tropomi_offl_no2_sum
        meta : dict
    ...
    def init_md(self):
        ...
        add = self.add_md
        add("""---
title:  '{target} Validation Appendices'
subtitle: "{source} vs {target} {spc}"
author:
- {author} ({institution})
maxwidth: 80%
...
""".format(**self.meta, source=source, spc=spc, target=target))
        add(f'{source} vs {target} {spc}', 'Title',)
    ...
    def add_loc(self, lockey, label, desc=None, headlevel=2):
        """
        ...
        if headlevel > 1:
            add(f'{label}', f'Heading {headlevel - 1}')
        if desc is None:
        ...
        for bykey in bykeys:
            ...
            for plotopt in plotopts:
                ...
                if os.path.exists(figpath):
                    if not found:
                        found = True
                        add(f'{label} by {bykey[2:]}', f'Heading {headlevel}')
    ...
    def from_keys(cls, xkey, ykey, locs=None, meta=None):
        ...
        rpt.init_md()
        rpt.add_maps()
        headlevel = min(1, len(locs)) + 1
        rpt.add_loc('summary', 'All Sites', 'all sites', headlevel=headlevel)
        for lockey in locs:
